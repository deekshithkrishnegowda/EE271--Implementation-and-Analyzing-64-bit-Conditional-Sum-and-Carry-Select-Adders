module ripple_carry_adder(input [7:0] a,b,input add_sub,clock,start,reset, output reg [7:0] sum,output cout,
				output reg complete);
...
fulladder FA0(.A(A_temp[0]),.B(d0),.Cin(add_sub),.sum(sum_temp[0]),.carry(cout0));
...
always@(negedge clock)
begin
		if((|sum_temp) | (!sum_temp))		//complete signal is made high by reduction operation on sum
		complete<=1'b1;
	else
		complete<=1'b0;
